...
async function runPowershellScript(commitHash: string, repoPath: string): Promise<{ success: boolean; stdout: string; stderr: string; code: number | null; errorMessage?: string }> {
...
server.tool(
  "get_git_merge_diff",
  "Generates the text diff for a Git merge commit against its first parent within a specified local repository.", // Updated description
  { // *** MODIFIED schema: Added repoPath ***
  ...
  async ({ commitHash, repoPath }): Promise<CallToolResult> => {
